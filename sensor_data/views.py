    ...
    try:
        ...
        if not is_user_valid(params['username'], params['password']):
            ...
            try:
                ...
                for idx, column in enumerate(file_read):
                    ...
                    try:
                        if data_src == SRC_ACC_SP:
                            ...
                            timestamp, ema_order = values.split(" ")
                            new_raw_data = models.step_detector(username=participant, timestamp=timestamp, ema_order=ema_order, day_num=get_day_num(float(timestamp), reg_time))
                            new_raw_data.save()
                        elif data_src == SRC_SP_UNLOCKED_DUR:
                            start, end, duration, ema_order = values.split(" ")
                            new_raw_data = models.unlocked_dur(username=participant, timestamp_start=start, timestamp_end=end, duration=duration, ema_order=ema_order, day_num=get_day_num(float(end), reg_time))
                            new_raw_data.save()
                        elif data_src == SRC_SP_SCREEN_ON_DUR:
                            start, end, duration, ema_order = values.split(" ")
                            new_raw_data = models.screen_on_dur(username=participant, timestamp_start=start, timestamp_end=end, duration=duration, ema_order=ema_order, day_num=get_day_num(float(end), reg_time))
                            new_raw_data.save()
                        elif data_src == SRC_SP_STATIONARY_DUR:
                            start, end, duration, ema_order = values.split(" ")
                            new_raw_data = models.stationary_dur(username=participant, timestamp_start=start, timestamp_end=end, duration=duration, ema_order=ema_order, day_num=get_day_num(float(end), reg_time))
                            new_raw_data.save()
                        elif data_src == SRC_SP_PHONE_CALLS:
                            start, end, call_type, duration, ema_order = values.split(" ")
                            new_raw_data = models.phone_calls(username=participant, timestamp_start=start, timestamp_end=end, call_type=call_type, duration=duration, ema_order=ema_order, day_num=get_day_num(float(end), reg_time))
                            new_raw_data.save()
                        elif data_src == SRC_SP_LIGHT:
                            timestamp, value, ema_order = values.split(" ")
                            new_raw_data = models.light_intensity(username=participant, timestamp=timestamp, value=value, ema_order=ema_order, day_num=get_day_num(float(timestamp), reg_time))
                            new_raw_data.save()
                        elif data_src == SRC_SP_GPS_LOCATIONS:
                            timestamp, lat, lng, accuracy, altitude, ema_order = values.split(" ")
                            new_raw_data = models.gps_locations(username=participant, timestamp=timestamp, lat=lat, lng=lng, accuracy=accuracy, altitude=altitude, ema_order=ema_order, day_num=get_day_num(float(timestamp), reg_time))
                            new_raw_data.save()
                        elif data_src == SRC_SP_ACTIVITY:
                            timestamp, activity_type, confidence, ema_order = values.split(" ")
                            new_raw_data = models.activities(username=participant, timestamp=timestamp, activity_type=activity_type, confidence=confidence, ema_order=ema_order, day_num=get_day_num(float(timestamp), reg_time))
                            new_raw_data.save()
                            ...
                            timestamp, value, ema_order = values.split(" ")
                            new_raw_data = models.audio_loudness(username=participant, timestamp=timestamp, value=value, ema_order=ema_order, day_num=get_day_num(float(timestamp), reg_time))
                            new_raw_data.save()
                        elif data_src == SRC_SP_ACTIVITY_DURATION:
                            start, end, activity_type, duration, ema_order = values.split(" ")
                            new_raw_data = models.activity_duration(username=participant, timestamp_start=start, timestamp_end=end, activity_type=activity_type, duration=duration, ema_order=ema_order, day_num=get_day_num(float(end), reg_time))
                            new_raw_data.save()
                ...
                participant.last_ds_smartphone = cur_datetime.timestamp()  # setting last data submission time

                participant.total_data_submitted += len(data_set) / 1024  # update total amount of data submitted
    ...
    try:
        ...
        if not is_user_valid(params['username'], params['password']):
            ...
            for element in params['app_usage'].split(','):
                package_name, last_time_used, total_time_in_foreground, ema_order = [int(value) if value.isdigit() else value for value in element.split(' ')]
